...
class MainWindow:
    ...
    def __init__(self, root, settings):
        ...
        # Если i18n не привязан к settings, создаём отдельно
    ...
    def _create_layout(self):
        ...
        # Превью - ПЕРЕДАЁМ СЕБЯ (self) В КАЧЕСТВЕ main_window
    ...
    def _create_input_panel(self, parent):
        """Создаёт панель ввода символов/иконок."""
    ...
    def _on_filename_template_change(self, event):
        """Обработчик изменения шаблона имени файла."""
    ...
    def _apply_settings(self):
        """Применяет настройки к виджетам."""
        # Язык и тема
    ...
    def _set_input_mode(self, is_icon_mode, apply=False):
        ...
        if is_icon_mode:
            ...
            if hasattr(self.preview, 'font_size_entry'):
                self.preview.font_size_entry.delete(0, "end")
                self.preview.font_size_entry.insert(0, str(size_val))
                if hasattr(self.preview, 'font_size_slider'):
                    self.preview.font_size_slider.set(size_val)
        else:
    ...
    def _default_icon_font_size(self):
        """Возвращает размер по умолчанию для иконок."""
    ...
    def _add_icon_paths(self, paths):
        ...
        if was_empty and self.settings.icon_mode:
            ...
            if dim:
                ...
                if hasattr(self.preview, 'font_size_entry'):
                    self.preview.font_size_entry.delete(0, "end")
                    self.preview.font_size_entry.insert(0, str(dim))
                    if hasattr(self.preview, 'font_size_slider'):
                        self.preview.font_size_slider.set(dim)
    ...
    def _cycle_case(self):
        """Циклически меняет регистр текста."""
    ...
    def _show_pattern_selector(self, patterns):
        """Показывает окно выбора паттерна."""
        import tkinter as tk
    ...
    def _on_generate(self):
        ...
        # Получаем размер
        try:
            font_size = int(self.preview.font_size_entry.get()) if hasattr(self.preview, 'font_size_entry') else 64
        except ValueError:
            font_size = 64
        ...
        def run_generation():
            try:
                if self.settings.icon_mode:
                    ...
                    count = len(parse_characters(raw))
